...
#-------------------------------------------------------------------------------
unit class Gnome::Gtk3::DrawingArea:auth<github:MARTIMM>:api<1>;
also is Gnome::Gtk3::Widget;
#-------------------------------------------------------------------------------
...
method _fallback ( $native-sub is copy --> Callable ) {
  ...
  if ?$s {
    Gnome::N::deprecate(
      "gtk-drawing-area-$native-sub", $new-patt, '0.47.4', '0.50.0'
    );
  }
    ...
    if ?$s {
      Gnome::N::deprecate(
        "gtk_$native-sub", $new-patt.subst('drawing-area-'),
        '0.47.4', '0.50.0'
      );
    }
      ...
      if ?$s {
        Gnome::N::deprecate(
          "$native-sub", $new-patt.subst('gtk-drawing-area-'),
          '0.47.4', '0.50.0'
        );
      }
    }
