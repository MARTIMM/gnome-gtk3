...
3. Then the event is pushed onto a stack so you can query the currently handled event with C<current-event()>.
...
=item Deletion and destruction events are still sent to the event widget for obvious reasons.
=item Events which directly relate to the visual representation of the event widget.
=item Leave events are delivered to the event widget if there was an enter event delivered to it before without the paired leave event.
=item Drag events are not redirected because it is unclear what the semantics of that would be.

=comment Another point of interest might be that all key events are first passed through the key snooper functions if there are any. Read the description of C<gtk-key-snooper-install()> if you need this feature.
...
method do-event ( N-GdkEvent $event ) {
  gtk_main_do_event( self._get-native-object-no-reffing, $event);
}
