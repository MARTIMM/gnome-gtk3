...
submethod BUILD ( *%options ) {
  ...
  if ? %options<native-object> || ? %options<widget> || %options<build-id> {
    ...
    self._set-native-object(gtk_cell_renderer_pixbuf_new());
  }
...
method _fallback ( $native-sub is copy --> Callable ) {
  ...
  if ?$s {
    Gnome::N::deprecate(
      "gtk_cell_renderer_pixbuf_$native-sub", $new-patt, '0.47.4', '0.50.0'
    );
  }
    ...
    if ?$s {
      Gnome::N::deprecate(
        "gtk_$native-sub", $new-patt.subst('cell-renderer-pixbuf-'),
        '0.47.4', '0.50.0'
      );
    }
      ...
      if ?$s {
        Gnome::N::deprecate(
          "$native-sub", $new-patt.subst('gtk-cell-renderer-pixbuf-'),
          '0.47.4', '0.50.0'
        );
      }
    }
