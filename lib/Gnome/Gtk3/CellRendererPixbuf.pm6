...
submethod BUILD ( *%options ) {
  ...
  if ? %options<empty> {
    self.native-gobject(gtk_cell_renderer_pixbuf_new());
  }
  ...
  self.set-class-info('GtkCellRendererPixbuf');
}
...
method _fallback ( $native-sub is copy --> Callable ) {
  ...
  self.set-class-name-of-sub('GtkCellRendererPixbuf');
  $s = callsame unless ?$s;
...
#-------------------------------------------------------------------------------
#TM:2:gtk_cell_renderer_pixbuf_new:new(:empty)
=begin pod
