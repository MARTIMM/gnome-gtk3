...
unit class Gnome::Gtk3::Enums:auth<github:MARTIMM>:api<1>;
...
Represents the orientation of widgets and other objects which can be switched between horizontal and vertical orientation on the fly, like B<Gnome::Gtk3::Toolbar> or B<Gnome::Gtk3::GesturePan>.
...
Note that many themes do not differentiate the appearance of the various shadow types: Either their is no visible shadow (I<GTK_SHADOW_NONE>), or there is (any other value).
...
=item GTK_STATE_FLAG_DIR_LTR: Widget is in left-to-right text direction.
=item GTK_STATE_FLAG_DIR_RTL: Widget is in right-to-left text direction.
=item GTK_STATE_FLAG_LINK: Widget is a link.
=item GTK_STATE_FLAG_VISITED: The location the widget points to has already been visited.
=item GTK_STATE_FLAG_CHECKED: Widget is checked.
=item GTK_STATE_FLAG_DROP_ACTIVE: Widget is highlighted as a drop target for DND.
