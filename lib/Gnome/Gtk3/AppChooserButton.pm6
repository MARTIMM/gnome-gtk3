...
The B<Gnome::Gtk3::AppChooserButton> is a widget that lets the user select 
an application. It implements the B<Gnome::Gtk3::AppChooser> interface. 
 
Initially, a B<Gnome::Gtk3::AppChooserButton> selects the first application 
in its list, which will either be the most-recently used application 
or, if  I<show-default-item> is C<True>, the 
default application. 
 
The list of applications shown in a B<Gnome::Gtk3::AppChooserButton> includes 
the recommended applications for the given content type. When 
 I<show-default-item> is set, the default application 
is also included. To let the user chooser other applications, 
you can set the  I<show-dialog-item> property, 
which allows to open a full B<Gnome::Gtk3::AppChooserDialog>. 
...
Create a new AppChooserButton object.
...
submethod BUILD ( *%options ) {
  ...
  if self.^name eq 'Gnome::Gtk3::AppChooserButton' #`{{ or %options<GtkAppChooserButton> }} {
    ...
    # process all other options
      ...
      if ? %options<___x___> {
        ...
        #$no = _gtk_app_chooser_button_new___x___($no);
      }
