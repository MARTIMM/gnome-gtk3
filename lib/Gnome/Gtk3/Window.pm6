...
=item N-GObject $screen; a B<Gnome::Gtk3::Screen> to draw to
...
Returns the B<Gnome::Gtk3::Screen> associated with I<window>.
...
method set-default ( $default_widget is copy ) {
  $default_widget .= get-native-object-no-reffing unless $default_widget ~~ N-GObject;
...
=item N-GList $list; (element-type GdkPixbuf) (transfer container): a list of B<Gnome::Gtk3::Pixbuf>
=end pod
...
Sets the B<Gnome::Gtk3::Screen> where the I<window> is displayed; if the window is already mapped, it will be unmapped, and then remapped on the new screen.
