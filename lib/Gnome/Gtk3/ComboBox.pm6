...
=SUBTITLE A widget used to choose from a list of items

=head1 Description


A C<Gnome::Gtk3::ComboBox> is a widget that allows the user to choose from a list of valid choices. The C<Gnome::Gtk3::ComboBox> displays the selected choice. When activated, the C<Gnome::Gtk3::ComboBox> displays a popup which allows the user to make a new choice. The style in which the selected value is displayed, and the style of the popup is determined by the current theme. It may be similar to a Windows-style combo box.

The C<Gnome::Gtk3::ComboBox> uses the model-view pattern; the list of valid choices is specified in the form of a tree model, and the display of the choices can be adapted to the data in the model by using cell renderers, as you would in a tree view. This is possible since C<Gnome::Gtk3::ComboBox> implements the C<Gnome::Gtk3::CellLayout> interface. The tree model holding the valid choices is not restricted to a flat list, it can be a real tree, and the popup will reflect the tree structure.

To allow the user to enter values not in the model, the “has-entry” property allows the C<Gnome::Gtk3::ComboBox> to contain a C<Gnome::Gtk3::Entry>. This entry can be accessed by calling C<gtk_bin_get_child()> on the combo box.

For a simple list of textual choices, the model-view API of C<Gnome::Gtk3::ComboBox> can be a bit overwhelming. In this case, C<Gnome::Gtk3::ComboBoxText> offers a simple alternative. Both C<Gnome::Gtk3::ComboBox> and C<Gnome::Gtk3::ComboBoxText> can contain an entry.
...
A C<Gnome::Gtk3::ComboBox> with an entry has a single CSS node with name combobox. It contains a bx with the .linked class and that box contains an entry and a button, both with the .combo class added. The button also contains another node with name arrow.

=head2 See Also

C<Gnome::Gtk3::ComboBoxText>, C<Gnome::Gtk3::TreeModel>, C<Gnome::Gtk3::CellRenderer>
...
Creates a new empty C<Gnome::Gtk3::ComboBox>.

Returns: A new C<Gnome::Gtk3::ComboBox>.
...
Creates a new empty C<Gnome::Gtk3::ComboBox> using I<area> to layout cells.

Returns: A new C<Gnome::Gtk3::ComboBox>.

  method gtk_combo_box_new_with_area ( N-GObject $area --> N-GObject  )

=item N-GObject $area; the C<Gnome::Gtk3::CellArea> to use to layout cell renderers
...
Creates a new empty C<Gnome::Gtk3::ComboBox> with an entry.
...
Creates a new C<Gnome::Gtk3::ComboBox> with the model initialized to I<model>.
...
=item N-GObject $model; A C<Gnome::Gtk3::TreeModel>.
...
`path` is the C<Gnome::Gtk3::TreePath> of the active item.
...
=item GtkTreeIter $iter; (out): The uninitialized C<Gnome::Gtk3::TreeIter>
...
=item GtkTreeIter $iter; (allow-none): The C<Gnome::Gtk3::TreeIter>, or C<Any>
...
=item N-GObject $model; (allow-none): A C<Gnome::Gtk3::TreeModel>
...
Returns the C<Gnome::Gtk3::TreeModel> which is acting as data source for I<combo_box>.

Returns: (transfer none): A C<Gnome::Gtk3::TreeModel> which was passed
during construction.
...
=item GtkTreeViewRowSeparatorFunc $func; a C<Gnome::Gtk3::TreeViewRowSeparatorFunc>
...
are the only C<Gnome::Gdk3::Devices> able to send events to it.
...
The I<move-active signal> is a [keybinding signal][C<Gnome::Gtk3::BindingSignal>] which gets emitted to move the active selection.
...
for the current C<Gnome::Gtk3::ComboBox> model.
...
=item $path; the C<Gnome::Gtk3::TreePath> string from the combo box's current model to format text for
...
The C<Gnome::GObject::Value> type of property I<model> is C<G_TYPE_OBJECT>.
...
The C<Gnome::GObject::Value> type of property I<wrap-width> is C<G_TYPE_INT>.
...
The C<Gnome::GObject::Value> type of property I<row-span-column> is C<G_TYPE_INT>.
...
The C<Gnome::GObject::Value> type of property I<column-span-column> is C<G_TYPE_INT>.
...
where `path` is the C<Gnome::Gtk3::TreePath> of the active item.
...
The C<Gnome::GObject::Value> type of property I<has-frame> is C<G_TYPE_BOOLEAN>.
...
The C<Gnome::GObject::Value> type of property I<popup-shown> is C<G_TYPE_BOOLEAN>.
...
The C<Gnome::GObject::Value> type of property I<button-sensitivity> is C<G_TYPE_ENUM>.
...
The C<Gnome::GObject::Value> type of property I<has-entry> is C<G_TYPE_BOOLEAN>.
...
The C<Gnome::GObject::Value> type of property I<entry-text-column> is C<G_TYPE_INT>.
...
The C<Gnome::GObject::Value> type of property I<id-column> is C<G_TYPE_INT>.
...
The C<Gnome::GObject::Value> type of property I<active-id> is C<G_TYPE_STRING>.
...
The C<Gnome::GObject::Value> type of property I<popup-fixed-width> is C<G_TYPE_BOOLEAN>.
...
The C<Gnome::GObject::Value> type of property I<cell-area> is C<G_TYPE_OBJECT>.

The C<Gnome::Gtk3::CellArea> used to layout cell renderers for this combo box.

If no area is specified when creating the combo box with C<gtk_combo_box_new_with_area()>
a horizontally oriented C<Gnome::Gtk3::CellAreaBox> will be used.
