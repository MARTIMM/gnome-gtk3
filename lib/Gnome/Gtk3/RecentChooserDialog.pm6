...
#-------------------------------------------------------------------------------
unit class Gnome::Gtk3::RecentChooserDialog:auth<github:MARTIMM>:ver<0.1.0>;
also is Gnome::Gtk3::Dialog;
also does Gnome::Gtk3::RecentChooser;
...
submethod BUILD ( *%options ) {
  ...
  if self.^name eq 'Gnome::Gtk3::RecentChooserDialog' or
    ...
    self.set-class-info('GtkRecentChooserDialog');
  }
}
...
method _fallback ( $native-sub --> Callable ) {
  ...
  my Callable $s;
  try { $s = &::("gtk_recent_chooser_dialog_$native-sub"); };
# check for gtk_, gdk_, g_, pango_, cairo_ !!!
  ...
  self.set-class-name-of-sub('GtkRecentChooserDialog');
  $s = callsame unless ?$s;
