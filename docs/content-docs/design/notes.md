...
The following is a (not very exhaustive) list of points which make up the design of the Perl6 packages.

* Native objects are wrapped into Perl6 classes. The native objects are mostly created or imported using the `.new()` method of the class and stored in the class with `.native-gobject()`. In rare occasions, the user may retrieve the object by using `.get-native-gobject()`.
...
* The native subroutines are defined in their corresponding Perl6 classes. They are defined and looked up in such a way that they are usable as methods in those classes.
...
* There are native subroutines which need a native object as one of their arguments. The `gtk_grid_attach()` in **Gnome::Gtk3::Grid** is an example of such a routine. The declaration of the `gtk_grid_attach()` native sub is;
  ...
  ```
  However, the signatures of all subroutines are checked against the arguments provided, so it is possible to retrieve the native object hidden in the object when a perl6 type is noticed. So the example becomes more simple;
  ```
  my Gnome::Gtk3::Grid $grid .= new(:empty);
...
* Sometimes I had to stray away from the native function names because of the way the sub must be defined in perl6. Causes can be;
  ...
  * Many subroutines also return native objects. For some of them, the type is known and can therefore be returned in a Perl6 class object instead of a native object.
